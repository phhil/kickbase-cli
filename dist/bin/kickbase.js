...
function collectLeafCommands(root) {
    const groups = {};
...
function suggestTopLevelCommand(input, known) {
    const normalized = input.toLowerCase();
    const ranked = known
        .map((cmd) => {
        const c = cmd.toLowerCase();
        let score = levenshtein(normalized, c);
        if (c.startsWith(normalized) || normalized.startsWith(c))
            score -= 1;
...
program
    ...
    if (opts.baseUrl)
        client.setBaseUrl(String(opts.baseUrl));
    if (opts.noCache)
        disableCache();
});
...
program
    ...
    const scored = allCommands
        ...
        if (!query || tokenHits === 0)
            return null;
        let score = tokenHits * 10;
        if (cmd.includes(query))
            score += 60;
        if (cmd.startsWith(`kb ${query}`) || cmd.startsWith(query))
            score += 100;
...
program.action(async () => {
    ...
    if (isAgent) {
        ...
        for (const group of groupOrder) {
            if (printed.has(group) || smartCmds.has(group))
                continue;
...
const topLevelCommands = new Set(program.commands.flatMap(cmd => {
