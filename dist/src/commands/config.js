...
export function registerConfigCommands(program, client) {
    ...
    config
        ...
        if (!isAgent)
            console.log(`Default league set to: ${leagueId}`);
    ...
    config
        ...
        if (!isAgent)
            console.log(id ? `Default league: ${id}` : 'No default league set. Run: kb config set-league <leagueId>');
    ...
    config
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/config');
            if (!isAgent)
            ...
            respondError('kickbase config show', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    config
        .command('version')
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/config/version');
            if (!isAgent)
                console.log(JSON.stringify(result, null, 2));
            respond('kickbase config version', result, []);
        }
        catch (err) {
            respondError('kickbase config version', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    config
        .command('onboarding')
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/config/onboarding');
            if (!isAgent)
                console.log(JSON.stringify(result, null, 2));
            respond('kickbase config onboarding', result, []);
        }
        catch (err) {
            respondError('kickbase config onboarding', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    config
        .command('overview')
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/base/overview');
            if (!isAgent)
                console.log(JSON.stringify(result, null, 2));
            respond('kickbase config overview', result, []);
        }
        catch (err) {
            respondError('kickbase config overview', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    config
        .command('products')
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/products');
            if (!isAgent)
                console.log(JSON.stringify(result, null, 2));
            respond('kickbase config products', result, []);
        }
        catch (err) {
            respondError('kickbase config products', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    config
        .command('promotion')
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/promotion');
            if (!isAgent)
                console.log(JSON.stringify(result, null, 2));
            respond('kickbase config promotion', result, []);
        }
        catch (err) {
            respondError('kickbase config promotion', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
}
