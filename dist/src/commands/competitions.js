...
export function registerCompetitionCommands(program, client) {
    const comp = program.command('competitions').description('Competition data (Bundesliga etc.)');
    const fmtMoney = (n) => typeof n === 'number' ? `${(n / 1_000_000).toFixed(1)}M` : '?';
    const posLabel = (p) => ({ 1: 'GK', 2: 'DEF', 3: 'MID', 4: 'FWD' }[Number(p)] ?? String(p ?? '?'));
    comp
        .command('list')
        .description('List all available competitions')
        .action(async () => {
        try {
            const result = await client.get('/v4/competitions');
            ...
            respondError('kickbase competitions list', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    comp
        ...
        try {
            const all = await client.get('/v4/competitions');
            const result = (all.it ?? []).find((c) => String(c.i) === String(competitionId));
            if (!result) {
            ...
            respondError('kickbase competitions overview', err.message, 'FETCH_FAILED', `Check that competition ID "${competitionId}" is valid.`);
        }
    });
    comp
        ...
        .action(async (competitionId) => {
        try {
            const result = await client.get(`/v4/competitions/${competitionId}/matchdays`);
            if (!isAgent && Array.isArray(result.it)) {
                console.log(`Matchdays: ${result.it.length} (current: ${result.day ?? '?'})`);
                const current = result.it.find((d) => Number(d.day) === Number(result.day)) ?? result.it[0];
                if (current?.it) {
            ...
            respondError('kickbase competitions matchdays', err.message, 'FETCH_FAILED', `Check that competition ID "${competitionId}" is valid.`);
        }
    });
    comp
        ...
        .action(async (competitionId) => {
        try {
            const result = await client.get(`/v4/competitions/${competitionId}/table`);
            if (!isAgent && result.it) {
                const sorted = [...result.it].sort((a, b) => (a.cpl ?? 999) - (b.cpl ?? 999));
                formatTable(sorted.map((t) => ({
                    rank: t.cpl ?? '?',
                    team: t.tn,
                    mp: t.mc ?? '?',
                    pts: t.cp ?? '?',
            ...
            respondError('kickbase competitions table', err.message, 'FETCH_FAILED', `Check that competition ID "${competitionId}" is valid.`);
        }
    });
    comp
        ...
        .action(async (competitionId) => {
        try {
            const result = await client.get(`/v4/competitions/${competitionId}/ranking`);
            if (!isAgent && result.it) {
                const sorted = [...result.it].sort((a, b) => (a.spl ?? 999) - (b.spl ?? 999));
                formatTable(sorted.map((t) => ({
                    rank: t.spl ?? '?',
                    team: t.tn,
                    pts: t.sp ?? '?',
            ...
            respondError('kickbase competitions ranking', err.message, 'FETCH_FAILED', `Check that competition ID "${competitionId}" is valid.`);
        }
    });
    // --- Players ---
    comp
        ...
        try {
            ...
            const result = await client.get(`/v4/competitions/${competitionId}/players${query}`);
            if (!isAgent && result.it) {
            ...
            respondError('kickbase competitions players', err.message, 'FETCH_FAILED', `Check that competition ID "${competitionId}" is valid.`);
        }
    });
    comp
        ...
        try {
            ...
            if (opts.league)
                params.set('leagueId', opts.league);
            if (opts.query)
                params.set('query', opts.query);
            params.set('start', opts.start);
            params.set('max', opts.max);
            const result = await client.get(`/v4/competitions/${competitionId}/players/search?${params}`);
            if (!isAgent && result.it) {
                formatTable(result.it.map((p) => ({
                    id: p.pi ?? p.i,
                    name: p.n,
            ...
            respondError('kickbase competitions players-search', err.message, 'FETCH_FAILED', `Check that competition ID "${competitionId}" is valid.`);
        }
    });
    comp
        .command('player <competitionId> <playerId>')
        .description('Get individual player details in a competition')
        .action(async (competitionId, playerId) => {
        try {
            const result = await client.get(`/v4/competitions/${competitionId}/players/${playerId}`);
            if (!isAgent) {
            ...
            respondError('kickbase competitions player', err.message, 'FETCH_FAILED', 'Check that competition and player IDs are valid.');
        }
    });
    comp
        ...
        .action(async (competitionId, playerId) => {
        try {
            const result = await client.get(`/v4/competitions/${competitionId}/players/${playerId}/performance`);
            if (!isAgent && Array.isArray(result.it)) {
                for (const season of result.it.slice(0, 3)) {
                    const games = Array.isArray(season.ph) ? season.ph.length : 0;
                    const recent = (season.ph ?? []).filter((g) => g.p !== undefined).slice(-3);
                    const recentText = recent.map((g) => `MD${g.day}:${g.p}`).join(', ');
            ...
            respondError('kickbase competitions player-performance', err.message, 'FETCH_FAILED', 'Check that competition and player IDs are valid.');
        }
    });
    comp
        ...
        .option('--day <dayNumber>', 'Filter by matchday')
        .action(async (competitionId, playerId, opts) => {
        try {
            const query = opts.day ? `?dayNumber=${opts.day}` : '';
            const result = await client.get(`/v4/competitions/${competitionId}/playercenter/${playerId}${query}`);
            if (!isAgent) {
            ...
            respondError('kickbase competitions player-events', err.message, 'FETCH_FAILED', 'Check that competition and player IDs are valid.');
        }
    });
    comp
        ...
        .action(async (competitionId, playerId, opts) => {
        try {
            const result = await client.get(`/v4/competitions/${competitionId}/players/${playerId}/marketvalue/${opts.timeframe}`);
            if (!isAgent) {
            ...
            respondError('kickbase competitions player-marketvalue', err.message, 'FETCH_FAILED', 'Check that competition and player IDs are valid.');
        }
    });
    // --- Teams ---
    comp
        ...
        .action(async (competitionId, teamId, opts) => {
        try {
            const query = opts.day ? `?dayNumber=${opts.day}` : '';
            const result = await client.get(`/v4/competitions/${competitionId}/teams/${teamId}/teamcenter${query}`);
            if (!isAgent) {
            ...
            respondError('kickbase competitions team-matchday', err.message, 'FETCH_FAILED', 'Check that competition and team IDs are valid.');
        }
    });
    comp
        .command('team-profile <competitionId> <teamId>')
        .description('Get all players on a team')
        .action(async (competitionId, teamId) => {
        try {
            const result = await client.get(`/v4/competitions/${competitionId}/teams/${teamId}/teamprofile`);
            if (!isAgent && result.it) {
            ...
            respondError('kickbase competitions team-profile', err.message, 'FETCH_FAILED', 'Check that competition and team IDs are valid.');
        }
    });
}
