...
export function registerLiveCommands(program, client) {
    ...
    live
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/live/eventtypes');
            if (!isAgent && Array.isArray(result.it)) {
                const rows = result.it
                    .map((e) => ({ code: e.i, name: e.ti }))
            ...
            respondError('kickbase live eventtypes', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
}
