...
export function registerChatCommands(program, client) {
    ...
    chat
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/chat/leagueselection');
            ...
            respondError('kickbase chat leagues', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    chat
        ...
        .action(async () => {
        try {
            const result = await client.get('/v4/chat/refreshtoken');
            if (!isAgent) {
            ...
            respondError('kickbase chat refresh-token', err.message, 'FETCH_FAILED', 'Ensure you are logged in. Run: kickbase user login');
        }
    });
    chat
        ...
        try {
            const result = await client.get(`/v4/chat/leagueselection/${leagueId}/users`);
            ...
            respondError('kickbase chat users', err.message, 'FETCH_FAILED', `Check that league ID "${leagueId}" is valid.`);
        }
    });
}
