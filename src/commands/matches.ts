...
function eventLabel(e: any): string {
  ...
  if (code === 1) return 'Goal';
  if (code === 3) return 'Assist';
  if (code === 4) return 'Card';
  if (code === 8) return 'Sub on';
  if (code === 9) return 'Sub off';
  if (code === 10) return 'Kickoff';
  if (code === 11) return 'Halftime';
  if (code === 12) return '2nd half';
  if (code === 13) return 'Full time';
  return `Event ${e.ke ?? '?'}`;
}

export function registerMatchCommands(program: Command, client: KickbaseClient) {
  ...
  matches
    ...
    .action(async (matchId: string) => {
      try {
        ...
        respondError('kickbase matches details', err.message, 'FETCH_FAILED',
          `Check that match ID "${matchId}" is valid.`);
      }
    });
  ...
  matches
    ...
    .action(async (matchId: string) => {
      try {
        const result = await client.get<any>(`/v4/matches/${matchId}/betlink`);
        if (!isAgent) {
        ...
        respondError('kickbase matches betlink', err.message, 'FETCH_FAILED',
          `Check that match ID "${matchId}" is valid.`);
      }
    });
}
