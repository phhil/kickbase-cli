...
export function registerBaseCommands(program: Command, client: KickbaseClient) {
  ...
  base
    ...
    .action(async (competitionId: string) => {
      try {
        const result = await client.get<any>(`/v4/base/news/${competitionId}`);
        if (!isAgent && result.it) {
          for (const item of result.it.slice(0, 15)) {
            console.log(`[${item.dt?.slice(0, 10) ?? ''}] ${item.ti ?? ''}`);
            if (item.rl) console.log(`  ${item.rl}`);
          }
          if (result.it.length > 15) console.log(`\n... and ${result.it.length - 15} more`);
        ...
        respondError('kickbase base news', err.message, 'FETCH_FAILED',
          'Check competition ID. Use 1 for Bundesliga, 2 for 2. Bundesliga.');
      }
    });
  ...
  base
    ...
    .action(async (competitionId: string) => {
      try {
        const result = await client.get<any>(`/v4/base/news/permanent/${competitionId}`);
        ...
        respondError('kickbase base news-permanent', err.message, 'FETCH_FAILED',
          'Check competition ID. Use 1 for Bundesliga, 2 for 2. Bundesliga.');
      }
    });
  ...
  base
    ...
    .action(async (competitionId: string) => {
      try {
        const result = await client.get<any>(`/v4/base/predictions/teams/${competitionId}`);
        ...
        respondError('kickbase base predictions', err.message, 'FETCH_FAILED',
          'Check competition ID. Use 1 for Bundesliga, 2 for 2. Bundesliga.');
      }
    });
  ...
  base
    ...
    .action(async () => {
      try {
        ...
        respondError('kickbase base stage', err.message, 'FETCH_FAILED',
  ...
  base
    ...
    .action(async (itemId: string) => {
      try {
        const result = await client.get<any>(`/v4/base/items/${itemId}`);
        if (!isAgent) {
          console.log(result.ti ?? itemId);
          if (result.dt) console.log(`Date: ${result.dt}`);
          if (result.rl) console.log(`Teaser: ${result.rl}`);
        ...
        respondError('kickbase base item', err.message, 'FETCH_FAILED',
          `Check that item ID "${itemId}" is valid.`);
      }
    });
  ...
  base
    ...
    .action(async (itemId: string) => {
      try {
        const result = await client.post<any>(`/v4/base/items/${itemId}/click`);
        if (!isAgent) console.log('Click tracked.');
        respond('kickbase base item-click', result, []);
      } catch (err: any) {
        respondError('kickbase base item-click', err.message, 'ACTION_FAILED',
          `Check that item ID "${itemId}" is valid.`);
      }
    });
}
