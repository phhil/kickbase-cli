...
import {
  ...
  fetchTable,
  TEAMS,
} from '../lib/ligainsider.js';

export function registerLigaInsiderCommands(program: Command, client: KickbaseClient) {
  ...
  li
    ...
    .action(async (opts: any) => {
      try {
        ...
        (result as any)._printed = !isAgent;
        ...
        respondError('kickbase li news', err.message, 'SCRAPE_FAILED',
          'Check your internet connection. LigaInsider.de may be temporarily unavailable.');
      }
    });
  ...
  li
    ...
    .action(async (teamInput: string) => {
      try {
        ...
        if (!isAgent) {
          console.log(`\n  === ${data.team} ===`);
          if (data.nextMatch) console.log(`  Next: ${data.nextMatch}`);
          console.log();
        ...
        respondError('kickbase li team', err.message, 'SCRAPE_FAILED',
          `Valid teams: ${Object.keys(TEAMS).join(', ')}`);
      }
    });
  ...
  li
    ...
    .action(async (opts: any) => {
      try {
        ...
        respondError('kickbase li injuries', err.message, 'SCRAPE_FAILED',
  ...
  li
    ...
    .action(async () => {
      try {
        ...
        respondError('kickbase li table', err.message, 'SCRAPE_FAILED',
          'Check your internet connection.');
      }
    });
  ...
  li
    ...
    .action(async (opts: any) => {
      try {
        ...
        if (!leagueId) {
          respondError('kickbase li alpha', 'No league ID provided', 'MISSING_LEAGUE',
            'Pass --league <id> or set KICKBASE_LEAGUE_ID. Find your league ID with: kickbase leagues list');
        }

        // Fetch Kickbase squad and LigaInsider injuries in parallel
        const [squad, allInjuries] = await Promise.all([
          client.get<any>(`/v4/leagues/${leagueId}/squad`),
          fetchAllInjuries(),
        ]);

        // Build a flat set of injured player names (lowercase) for matching
        const injuredMap = new Map<string, { reason: string; duration: string; team: string }>();
        ...
        respondError('kickbase li alpha', err.message, 'ALPHA_FAILED',
          'Ensure you are logged in and league ID is correct.');
      }
    });
  ...
  li
    ...
    .action(async () => {
      try {
        ...
        respondError('kickbase li scan', err.message, 'SCAN_FAILED',
          'Check your internet connection.');
      }
    });
}
