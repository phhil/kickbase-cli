...
import { respond, respondError, isAgent, formatTable, formatKeyValue, truncateResult } from '../lib/envelope.js';
import { resolveLeagueId } from '../lib/config.js';
import { c } from '../lib/colors.js';
import { POSITION, PROB, STATUS, STATUS_SHORT, decodeEvents, decodeEventsText, formatSeconds } from '../lib/mappings.js';

export function registerLeagueCommands(program: Command, client: KickbaseClient) {
  const leagues = program.command('leagues').description('League management, market, lineup, players');
  const fmtMoney = (n: number | undefined | null) => (typeof n === 'number' ? c.money(n) : '?');
  const fmtDate = (s: string | undefined | null) => (s ? new Date(s).toLocaleString() : '?');
  const posLabel = (pos: number | undefined) => POSITION[pos ?? -1] ?? String(pos ?? '?');
  const probLabel = (prob: number | undefined) => PROB[prob ?? -1] ?? String(prob ?? '?');
  const statusLabel = (st: number | undefined) => STATUS_SHORT[st ?? -1] ?? STATUS[st ?? -1] ?? String(st ?? '?');
  ...
  leagues
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/leagues/selection');
        if (!isAgent && result.it) {
          formatTable(result.it.map((l: any) => ({
            id: l.i,
            name: l.n,
            managers: l.pl ?? '?',
            squad: l.clpc ?? l.lpc ?? '?',
        ...
        respond('kickbase leagues list', { ...result, _printed: !isAgent }, [
          ...(result.it ?? []).slice(0, 3).map((l: any) => ({
            command: `kickbase leagues overview ${l.i}`,
            description: `View league "${l.n ?? l.i}"`,
          })),
        ]);
      } catch (err: any) {
        respondError('kickbase leagues list', err.message, 'FETCH_FAILED',
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const query = opts.includeManagers ? '?includeManagersAndBattles=true' : '';
        const result = await client.get<any>(`/v4/leagues/${lid}/overview${query}`);
        if (!isAgent) {
        ...
        respondError('kickbase leagues overview', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/settings`);
        ...
        respondError('kickbase leagues settings', err.message, 'FETCH_FAILED',
          'You may not have admin access to this league.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/settings/managers`);
        if (!isAgent && result.it) {
        ...
        respondError('kickbase leagues settings-managers', err.message, 'FETCH_FAILED',
          'You may not have admin access to this league.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        ...
        const result = await client.get<any>(`/v4/leagues/${lid}/ranking${query}`);
        const users = result.us ?? result.it ?? [];
        if (!isAgent && users.length > 0) {
          const sortedUsers = [...users].sort((a: any, b: any) => (a.spl ?? a.pl ?? 999) - (b.spl ?? b.pl ?? 999));
          formatTable(sortedUsers.map((u: any) => ({
            rank: u.spl ?? u.pl ?? '?',
            name: u.n,
            points: u.sp ?? '?',
            md: u.mdp ?? '?',
        ...
        respondError('kickbase leagues ranking', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/me`);
        if (!isAgent) {
          ...
          if (Array.isArray(result.tpc) && result.tpc.length > 0) {
            console.log('\n  Players by Team');
            formatTable(result.tpc.map((t: any) => ({
              teamId: t.tid,
              count: t.npt ?? 0,
            })), [
        ...
        respondError('kickbase leagues me', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/me/budget`);
        if (!isAgent) {
          console.log(c.header('\n  Budget\n'));
          formatKeyValue({
        ...
        respondError('kickbase leagues budget', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/squad`);
        ...
        if (opts.details && players.length > 0) {
          ...
          const enriched = players.map((p: any, idx: number) => {
            const d = details[idx];
            return {
              ...p,
              tn: d?.tn ?? '',
        ...
        respondError('kickbase leagues squad', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/lineup`);
        if (!isAgent) {
          const players = result.it ?? [];
          const formationCounts = players.reduce((acc: Record<string, number>, p: any) => {
          ...
          formatTable(players
            .slice()
            .sort((a: any, b: any) => (a.lo ?? 99) - (b.lo ?? 99))
            .map((p: any) => {
              const recent = Array.isArray(p.ph)
                ? p.ph.filter((e: any) => e?.hp && typeof e.p === 'number').map((e: any) => e.p)
                : [];
              const recentAvg = recent.length ? (recent.reduce((s: number, n: number) => s + n, 0) / recent.length).toFixed(0) : '-';
              return {
                slot: p.lo ?? 'B',
                name: p.n,
                pos: posLabel(p.pos),
        ...
        respondError('kickbase leagues lineup', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        ...
        const result = await client.post<any>(`/v4/leagues/${lid}/lineup`, body);
        if (!isAgent) console.log('Lineup updated.');
        respond('kickbase leagues lineup-set', result, [
          { command: `kickbase leagues lineup ${lid}`, description: 'View updated lineup' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues lineup-set', err.message, 'ACTION_FAILED',
          'Check your lineup data JSON format.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/lineup/overview`);
        if (!isAgent) {
          console.log(`Matchday: ${result.mdln ?? '?'} | Deadline: ${fmtDate(result.lis)} | Formation: ${result.t ?? '?'}`);
          if (result.lp) {
            formatTable(result.lp.map((p: any) => ({
              name: p.n,
              pos: posLabel(p.pos),
              avgPts: p.ap,
        ...
        respondError('kickbase leagues lineup-overview', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/lineup/selection`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        ...
        respondError('kickbase leagues lineup-selection', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/lineup/teams`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues lineup-teams', result, [
          { command: `kickbase leagues lineup-overview ${lid}`, description: 'View lineup overview' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues lineup-teams', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/lineup/fill`);
        if (!isAgent) console.log('Lineup autofilled.');
        respond('kickbase leagues lineup-fill', result, [
          { command: `kickbase leagues lineup-overview ${lid}`, description: 'View updated lineup' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues lineup-fill', err.message, 'ACTION_FAILED',
          'Autofill may not be available.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/lineup/clear`);
        if (!isAgent) console.log('Lineup cleared.');
        respond('kickbase leagues lineup-clear', result, [
          { command: `kickbase leagues lineup-overview ${lid}`, description: 'View lineup' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues lineup-clear', err.message, 'ACTION_FAILED',
          'Clear may not be available.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/teamcenter/myeleven`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues myeleven', result, [
          { command: `kickbase leagues squad ${lid}`, description: 'View full squad' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues myeleven', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/market`);
        if (!isAgent && result.it) {
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const body = JSON.parse(opts.data);
        const result = await client.post<any>(`/v4/leagues/${lid}/market`, body);
        if (!isAgent) console.log('Player listed on transfer market.');
        respond('kickbase leagues market-list', result, [
          { command: `kickbase leagues market ${lid}`, description: 'View market' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues market-list', err.message, 'ACTION_FAILED',
          'Check your transfer listing data format.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.delete<any>(`/v4/leagues/${lid}/market/${playerId}`);
        if (!isAgent) console.log(`Player ${playerId} removed from market.`);
        respond('kickbase leagues market-remove', result, [
          { command: `kickbase leagues market ${lid}`, description: 'View market' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues market-remove', err.message, 'ACTION_FAILED',
          'Check that player ID is valid and you own this player.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(
          `/v4/leagues/${lid}/market/${playerId}/offers`,
          { price: parseInt(opts.amount, 10) }
        );
        if (!isAgent) console.log(`Offer placed: ${opts.amount}`);
        respond('kickbase leagues market-offer', result, [
          { command: `kickbase leagues market ${lid}`, description: 'View market' },
          { command: `kickbase leagues budget ${lid}`, description: 'Check remaining budget' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues market-offer', err.message, 'ACTION_FAILED',
          'Check your budget and that the player is on the market.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string, offerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.delete<any>(`/v4/leagues/${lid}/market/${playerId}/offers/${offerId}`);
        if (!isAgent) console.log('Offer withdrawn.');
        respond('kickbase leagues market-withdraw', result, [
          { command: `kickbase leagues market ${lid}`, description: 'View market' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues market-withdraw', err.message, 'ACTION_FAILED',
          'Check that the offer ID is valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string, offerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/market/${playerId}/offers/${offerId}/accept`);
        if (!isAgent) console.log('Offer accepted.');
        respond('kickbase leagues market-accept', result, [
          { command: `kickbase leagues squad ${lid}`, description: 'View updated squad' },
          { command: `kickbase leagues budget ${lid}`, description: 'Check budget' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues market-accept', err.message, 'ACTION_FAILED',
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string, offerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/market/${playerId}/offers/${offerId}/decline`);
        if (!isAgent) console.log('Offer declined.');
        respond('kickbase leagues market-decline', result, [
          { command: `kickbase leagues market ${lid}`, description: 'View market' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues market-decline', err.message, 'ACTION_FAILED',
          'Check that the offer is still valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/market/${playerId}/sell`);
        if (!isAgent) console.log(`Player ${playerId} sold to Kickbase.`);
        respond('kickbase leagues market-sell', result, [
          { command: `kickbase leagues squad ${lid}`, description: 'View updated squad' },
          { command: `kickbase leagues budget ${lid}`, description: 'Check budget' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues market-sell', err.message, 'ACTION_FAILED',
          'Check that the player is listed and a Kickbase offer exists.');
      }
    });
    ...
    .action(async (leagueId: string | undefined, playerId: string) => {
      try {
        ...
        const [result, perf, ranking] = await Promise.all([
          client.get<any>(`/v4/leagues/${lid}/players/${playerId}`),
          client.get<any>(`/v4/leagues/${lid}/players/${playerId}/performance`).catch(() => null),
        ...
        if (!isAgent) {
          ...
          const ownerUser = result.oui !== '0' && ranking?.us
            ? ranking.us.find((u: any) => u.i === result.oui)
            : null;
          ...
          if (perf?.it) {
            const current = perf.it.find((s: any) => s.ti === '2025/2026') ?? perf.it[perf.it.length - 1];
            if (current?.ph) {
              const played = current.ph.filter((m: any) => m.p != null).slice(-10);
              if (played.length > 0) {
                console.log(c.header('\n  Recent Performance\n'));
                formatTable(played.map((m: any) => ({
                  day: `MD ${m.day}`,
                  pts: m.p,
                  min: m.mp,
        ...
        respondError('kickbase leagues player', err.message, 'FETCH_FAILED',
          'Check that league and player IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/players/${playerId}/performance`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        ...
        respondError('kickbase leagues player-performance', err.message, 'FETCH_FAILED',
          'Check that league and player IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/players/${playerId}/transfers`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues player-transfers', result, [
          { command: `kickbase leagues player ${lid} ${playerId}`, description: 'View player details' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues player-transfers', err.message, 'FETCH_FAILED',
          'Check that league and player IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string, opts: any) => {
      try {
        ...
        const result = await client.get<any>(`/v4/leagues/${lid}/players/${playerId}/transferHistory${query}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues player-transfer-history', result, [
          { command: `kickbase leagues player ${lid} ${playerId}`, description: 'View player details' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues player-transfer-history', err.message, 'FETCH_FAILED',
          'Check that league and player IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/players/${playerId}/marketvalue/${opts.timeframe}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues player-marketvalue', result, [
          { command: `kickbase leagues player ${lid} ${playerId}`, description: 'View player details' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues player-marketvalue', err.message, 'FETCH_FAILED',
          'Check that league and player IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/scoutedplayers`);
        if (!isAgent && result.it) {
          formatTable(result.it, [
            { key: 'i', label: 'ID', width: 14 },
            { key: 'n', label: 'Name', width: 25 },
        ...
        respondError('kickbase leagues scouted', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/scoutedplayers/${playerId}`);
        if (!isAgent) console.log(`Player ${playerId} added to scouted list.`);
        respond('kickbase leagues scout-add', result, [
          { command: `kickbase leagues scouted ${lid}`, description: 'View scouted players' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues scout-add', err.message, 'ACTION_FAILED',
          'Check that league and player IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, playerId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.delete<any>(`/v4/leagues/${lid}/scoutedplayers/${playerId}`);
        if (!isAgent) console.log(`Player ${playerId} removed from scouted list.`);
        respond('kickbase leagues scout-remove', result, [
          { command: `kickbase leagues scouted ${lid}`, description: 'View scouted players' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues scout-remove', err.message, 'ACTION_FAILED',
          'Check that league and player IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.delete<any>(`/v4/leagues/${lid}/scoutedplayers`);
        if (!isAgent) console.log('Scouted list cleared.');
        respond('kickbase leagues scout-clear', result, [
          { command: `kickbase leagues market ${lid}`, description: 'View transfer market' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues scout-clear', err.message, 'ACTION_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, userId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/managers/${userId}/dashboard`);
        if (!isAgent) {
          ...
          const fmtMoney = (v: number) => v != null ? `${(v / 1_000_000).toFixed(1)}M` : '?';
          ...
          if (result.ph?.length) {
            lines.push(['Season Points', result.ph.map((p: any) => p != null ? String(p) : '-').join(' → ')]);
          }
        ...
        respondError('kickbase leagues manager', err.message, 'FETCH_FAILED',
          'Check that league and user IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, userId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/managers/${userId}/performance`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        ...
        respondError('kickbase leagues manager-performance', err.message, 'FETCH_FAILED',
          'Check that league and user IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, userId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/managers/${userId}/squad`);
        if (!isAgent && result.it) {
          formatTable(result.it, [
            { key: 'i', label: 'ID', width: 14 },
            { key: 'n', label: 'Name', width: 25 },
        ...
        respondError('kickbase leagues manager-squad', err.message, 'FETCH_FAILED',
          'Check that league and user IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .option('--start <n>', 'Offset', '0')
    .action(async (leagueId: string | undefined, userId: string, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const query = `?start=${opts.start}`;
        const result = await client.get<any>(`/v4/leagues/${lid}/managers/${userId}/transfer${query}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues manager-transfers', result, [
          { command: `kickbase leagues manager ${lid} ${userId}`, description: 'View dashboard' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues manager-transfers', err.message, 'FETCH_FAILED',
          'Check that league and user IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, userId: string, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const query = opts.day ? `?dayNumber=${opts.day}` : '';
        const result = await client.get<any>(`/v4/leagues/${lid}/users/${userId}/teamcenter${query}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues manager-teamcenter', result, [
          { command: `kickbase leagues manager ${lid} ${userId}`, description: 'View dashboard' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues manager-teamcenter', err.message, 'FETCH_FAILED',
          'Check that league and user IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, teamId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/teams/${teamId}/teamprofile/`);
        if (!isAgent && result.it) {
          formatTable(result.it, [
            { key: 'i', label: 'ID', width: 14 },
            { key: 'n', label: 'Name', width: 25 },
        ...
        respondError('kickbase leagues team', err.message, 'FETCH_FAILED',
          'Check that league and team IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        ...
        const result = await client.get<any>(`/v4/leagues/${lid}/activitiesFeed?${params}`);
        if (!isAgent && result.it) {
          for (const item of result.it.slice(0, 20)) {
            console.log(`[${item.dt ?? ''}] ${item.txt ?? item.t ?? JSON.stringify(item)}`);
          }
          if (result.it.length > 20) console.log(`... and ${result.it.length - 20} more`);
        }
        ...
        respondError('kickbase leagues feed', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, activityId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/activitiesFeed/${activityId}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        ...
        respondError('kickbase leagues feed-item', err.message, 'FETCH_FAILED',
          'Check that league and activity IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, activityId: string, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/activitiesFeed/${activityId}`, {
          txt: opts.text,
        });
        if (!isAgent) console.log('Comment posted.');
        respond('kickbase leagues feed-comment', result, [
          { command: `kickbase leagues feed-comments ${lid} ${activityId}`, description: 'View comments' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues feed-comment', err.message, 'ACTION_FAILED',
          'Check that league and activity IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, activityId: string, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const params = new URLSearchParams({ start: opts.start, max: opts.max });
        const result = await client.get<any>(`/v4/leagues/${lid}/activitiesFeed/${activityId}/comments?${params}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues feed-comments', result, [
          { command: `kickbase leagues feed-comment ${lid} ${activityId} --text "..."`, description: 'Add comment' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues feed-comments', err.message, 'FETCH_FAILED',
          'Check that league and activity IDs are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/user/achievements`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues achievements', result, []);
      } catch (err: any) {
        respondError('kickbase leagues achievements', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, type: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/user/achievements/${type}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues achievement', result, [
          { command: `kickbase leagues achievements ${lid}`, description: 'View all achievements' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues achievement', err.message, 'FETCH_FAILED',
          'Check that league ID and achievement type are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, type: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/leagues/${lid}/battles/${type}/users`);
        if (!isAgent) {
          if (result.n) console.log(`${result.n}${result.d ? ` — ${result.d}` : ''}\n`);
          if (result.us) {
            formatTable(result.us.map((u: any) => ({
              place: u.pl,
        ...
        respondError('kickbase leagues battle', err.message, 'FETCH_FAILED',
          'Check that league ID and battle type are valid. Types: 1-8.');
      }
    });
      ...
      try {
        ...
        const battles = await Promise.all(
          [1, 2, 3, 4, 5, 6, 7, 8].map(t =>
            client.get<any>(`/v4/leagues/${lid}/battles/${t}/users`).catch(() => null)
          )
        );
        const summary = battles.filter(Boolean).map((b: any, i: number) => ({
          type: i + 1,
          name: b.n ?? `Battle ${i + 1}`,
          description: b.d ?? '',
        ...
        respondError('kickbase leagues battles', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/leagues');
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        ...
        respondError('kickbase leagues all', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  leagues
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/leagues/list');
        if (!isAgent && result.it) {
          formatTable(result.it, [
        ...
        respondError('kickbase leagues browse', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  leagues
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/leagues/recommended');
        if (!isAgent && result.it) {
          formatTable(result.it, [
            { key: 'i', label: 'ID', width: 14 },
            { key: 'lnm', label: 'Name', width: 25 },
        ...
        respondError('kickbase leagues recommended', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  leagues
    ...
    .action(async (opts: any) => {
      try {
        const body = JSON.parse(opts.data);
        const result = await client.post<any>('/v4/leagues', body);
        if (!isAgent) console.log(`League created! ID: ${result.i ?? JSON.stringify(result)}`);
        respond('kickbase leagues create', result, [
          { command: 'kickbase leagues list', description: 'View your leagues' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues create', err.message, 'ACTION_FAILED',
          'Check your league creation data format.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const path = opts.code
          ? `/v4/leagues/${lid}/join/${opts.code}`
          : `/v4/leagues/${lid}/join`;
        const result = await client.post<any>(path);
        if (!isAgent) console.log(`Joined league ${lid}.`);
        respond('kickbase leagues join', result, [
          { command: `kickbase leagues overview ${lid}`, description: 'View league overview' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues join', err.message, 'ACTION_FAILED',
          'Check that the league ID and invitation code are valid.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.get<any>(`/v4/invitations/${lid}/code`);
        if (!isAgent) console.log(`Invitation code: ${result.cd ?? result.code ?? JSON.stringify(result)}`);
        respond('kickbase leagues invite-code', result, []);
      } catch (err: any) {
        respondError('kickbase leagues invite-code', err.message, 'FETCH_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (code: string) => {
      try {
        const result = await client.get<any>(`/v4/invitations/${code}/validate`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase leagues invite-validate', result, []);
      } catch (err: any) {
        respondError('kickbase leagues invite-validate', err.message, 'FETCH_FAILED',
          `Check that invitation code "${code}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const body = JSON.parse(opts.data);
        const result = await client.post<any>(`/v4/leagues/${lid}/image`, body);
        if (!isAgent) console.log('League image updated.');
        respond('kickbase leagues image', result, []);
      } catch (err: any) {
        respondError('kickbase leagues image', err.message, 'ACTION_FAILED',
          'Check your image data format.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/setteamseen`);
        if (!isAgent) console.log('Teams marked as seen.');
        respond('kickbase leagues setteamseen', result, []);
      } catch (err: any) {
        respondError('kickbase leagues setteamseen', err.message, 'ACTION_FAILED',
          `Check that league ID "${leagueId ?? '(default)'}" is valid.`);
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/reset`);
        if (!isAgent) console.log('League reset.');
        respond('kickbase leagues admin-reset', result, []);
      } catch (err: any) {
        respondError('kickbase leagues admin-reset', err.message, 'ACTION_FAILED',
          'You may not have admin access to this league.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/resetteams`);
        if (!isAgent) console.log('Teams reset.');
        respond('kickbase leagues admin-resetteams', result, []);
      } catch (err: any) {
        respondError('kickbase leagues admin-resetteams', err.message, 'ACTION_FAILED',
          'You may not have admin access to this league.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, opts: any) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const body = JSON.parse(opts.data);
        const result = await client.post<any>(`/v4/leagues/${lid}/settings`, body);
        if (!isAgent) console.log('League settings updated.');
        respond('kickbase leagues admin-settings-update', result, []);
      } catch (err: any) {
        respondError('kickbase leagues admin-settings-update', err.message, 'ACTION_FAILED',
          'You may not have admin access to this league.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, userId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.post<any>(`/v4/leagues/${lid}/users/${userId}/unlock`);
        if (!isAgent) console.log(`User ${userId} unlocked.`);
        respond('kickbase leagues admin-unlock', result, []);
      } catch (err: any) {
        respondError('kickbase leagues admin-unlock', err.message, 'ACTION_FAILED',
          'You may not have admin access to this league.');
      }
    });
  ...
  leagues
    ...
    .action(async (leagueId: string | undefined, userId: string) => {
      try {
        const lid = resolveLeagueId(leagueId);
        const result = await client.delete<any>(`/v4/leagues/${lid}/users/${userId}`);
        if (!isAgent) console.log(`User ${userId} removed from league.`);
        respond('kickbase leagues kick', result, [
          { command: `kickbase leagues settings-managers ${lid}`, description: 'View members' },
        ]);
      } catch (err: any) {
        respondError('kickbase leagues kick', err.message, 'ACTION_FAILED',
          'You may not have permission to remove this user.');
      }
    });
}
