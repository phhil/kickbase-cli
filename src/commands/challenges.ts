...
import { respond, respondError, isAgent, formatTable, truncateResult } from '../lib/envelope.js';

export function registerChallengeCommands(program: Command, client: KickbaseClient) {
  ...
  challenges
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/overview');
        if (!isAgent && result.it) {
          formatTable(result.it.map((c: any) => ({
            id: c.ch ?? c.i,
            competition: c.cpi ?? '?',
        ...
        respond('kickbase challenges overview', result, [
          ...
          ...(result.it ?? []).slice(0, 3).map((c: any) => ({
            command: `kickbase challenges profile ${c.ch ?? c.i}`,
            description: `View challenge "${c.ch ?? c.i}"`,
          })),
        ]);
      } catch (err: any) {
        respondError('kickbase challenges overview', err.message, 'FETCH_FAILED',
  ...
  challenges
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/archive');
        if (!isAgent && result.it) {
          formatTable(result.it.map((c: any) => ({
            id: c.ch ?? c.i,
            name: c.chn ?? c.n ?? '',
        ...
        respondError('kickbase challenges archive', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  challenges
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/recommended');
        ...
        respondError('kickbase challenges recommended', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  challenges
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/selection');
        if (!isAgent) {
        ...
        respondError('kickbase challenges selection', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/profile`);
        if (!isAgent) {
          console.log(result.n ?? `Challenge ${challengeId}`);
          if (result.d) console.log(result.d);
          if (result.ruls?.b) console.log(`Budget: ${result.ruls.b}`);
          if (result.ruls?.mpst) console.log(`Max per club: ${result.ruls.mpst}`);
          if (Array.isArray(result.mds)) console.log(`Matchdays: ${result.mds.join(', ')}`);
          result._printed = true;
        }
        ...
        respondError('kickbase challenges profile', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/ranking`);
        ...
        respondError('kickbase challenges ranking', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string, opts: any) => {
      try {
        const query = opts.day ? `?dayNumber=${opts.day}` : '';
        const result = await client.get<any>(`/v4/challenges/${challengeId}/performance${query}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase challenges performance', result, [
          { command: `kickbase challenges ranking ${challengeId}`, description: 'View ranking' },
        ]);
      } catch (err: any) {
        respondError('kickbase challenges performance', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/top10`);
        if (!isAgent && result.it) {
        ...
        respondError('kickbase challenges top10', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  lineup
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/lineup/overview`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        ...
        respondError('kickbase challenges lineup overview', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  lineup
    ...
    .action(async (challengeId: string, opts: any) => {
      try {
        ...
        if (opts.sorting) params.set('sorting', opts.sorting);
        if (opts.query) params.set('query', opts.query);
        params.set('start', opts.start);
        params.set('max', opts.max);
        const result = await client.get<any>(`/v4/challenges/${challengeId}/lineup/selection?${params}`);
        if (!isAgent && result.it) {
          formatTable(result.it, [
        ...
        respondError('kickbase challenges lineup selection', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  lineup
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/lineup/teams`);
        if (!isAgent && result.it) {
          formatTable(result.it, [
            { key: 'i', label: 'ID', width: 12 },
        ...
        respondError('kickbase challenges lineup teams', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  lineup
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/lineup/livepitch`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase challenges lineup livepitch', result, [
          { command: `kickbase challenges lineup overview ${challengeId}`, description: 'View lineup' },
        ]);
      } catch (err: any) {
        respondError('kickbase challenges lineup livepitch', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  lineup
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.post<any>(`/v4/challenges/${challengeId}/lineup/fill`);
        if (!isAgent) console.log('Lineup autofilled successfully.');
        ...
        respondError('kickbase challenges lineup fill', err.message, 'ACTION_FAILED',
          'Autofill may not be available for this challenge.');
      }
    });
  ...
  lineup
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.post<any>(`/v4/challenges/${challengeId}/lineup/clear`);
        ...
        respondError('kickbase challenges lineup clear', err.message, 'ACTION_FAILED',
          'Clear may not be available for this challenge.');
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/table`);
        if (!isAgent && result.it) {
          formatTable(result.it, [
        ...
        respondError('kickbase challenges table', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string, userId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/table/${userId}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase challenges table-user', result, [
          { command: `kickbase challenges table ${challengeId}`, description: 'View full table' },
        ]);
      } catch (err: any) {
        respondError('kickbase challenges table-user', err.message, 'FETCH_FAILED',
          'Check that challenge and user IDs are valid.');
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/favorites`);
        if (!isAgent && result.it) {
          formatTable(result.it, [
            { key: 'i', label: 'ID', width: 12 },
            { key: 'n', label: 'Name', width: 25 },
        ...
        respondError('kickbase challenges favorites', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string, opts: any) => {
      try {
        ...
        params.set('start', opts.start);
        params.set('max', opts.max);
        const result = await client.get<any>(`/v4/challenges/${challengeId}/favorites/search?${params}`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        respond('kickbase challenges favorites-search', result, []);
      } catch (err: any) {
        respondError('kickbase challenges favorites-search', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (userId: string) => {
      try {
        const result = await client.post<any>('/v4/challenges/favorites', { ui: userId });
        if (!isAgent) console.log(`Manager ${userId} added to favorites.`);
        respond('kickbase challenges favorites-add', result, []);
      } catch (err: any) {
        respondError('kickbase challenges favorites-add', err.message, 'ACTION_FAILED',
          `Check that user ID "${userId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (userId: string) => {
      try {
        const result = await client.delete<any>(`/v4/challenges/favorites/${userId}`);
        if (!isAgent) console.log(`Manager ${userId} removed from favorites.`);
        respond('kickbase challenges favorites-remove', result, []);
      } catch (err: any) {
        respondError('kickbase challenges favorites-remove', err.message, 'ACTION_FAILED',
          `Check that user ID "${userId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/${challengeId}/perfectlineup`);
        if (!isAgent) console.log(JSON.stringify(result, null, 2));
        ...
        respondError('kickbase challenges perfectlineup', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  challenges
    ...
    .action(async (challengeId?: string) => {
      try {
        ...
        if (!isAgent) console.log('Joined challenge.');
        respond('kickbase challenges join', result, [
          { command: 'kickbase challenges overview', description: 'View your challenges' },
        ]);
      } catch (err: any) {
        respondError('kickbase challenges join', err.message, 'ACTION_FAILED',
          'Check that the challenge ID is valid and you meet join requirements.');
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/lobby/overview');
        if (!isAgent) {
          const dv = result.stg?.dv;
          if (dv) console.log(`Division: ${dv.dvn ?? dv.dvi} (${dv.prl ?? 0} points)`);
        ...
        respondError('kickbase challenges lobby overview', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/lobby/overview/live');
        if (!isAgent) {
        ...
        respondError('kickbase challenges lobby live', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/lobby/explore');
        ...
        respondError('kickbase challenges lobby explore', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  lobby
    ...
    .action(async (challengeId: string) => {
      try {
        const result = await client.get<any>(`/v4/challenges/lobby/explore/${challengeId}`);
        if (!isAgent) {
          console.log(result.chn ?? result.n ?? `Challenge ${challengeId}`);
          if (result.lis) console.log(`Deadline: ${result.lis}`);
          result._printed = true;
        }
        ...
        respondError('kickbase challenges lobby explore-challenge', err.message, 'FETCH_FAILED',
          `Check that challenge ID "${challengeId}" is valid.`);
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/lobby/divisionLadder');
        ...
        respondError('kickbase challenges lobby divisions', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/lobby/profile');
        if (!isAgent) {
          const u = result.u;
          const dv = result.dv;
          if (u) console.log(`${u.unm ?? 'You'}`);
          if (dv) console.log(`Division: ${dv.dvn ?? dv.dvi}`);
        ...
        respondError('kickbase challenges lobby profile', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/lobby/skillPoints/summary');
        if (!isAgent) {
        ...
        respondError('kickbase challenges lobby skillpoints', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.post<any>('/v4/challenges/lobby/skillPoints/collect');
        if (!isAgent) console.log('Skill points collected!');
        ...
        respondError('kickbase challenges lobby skillpoints-collect', err.message, 'ACTION_FAILED',
          'No skill points available to collect.');
      }
    });
  ...
  lobby
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/lobby/archive');
        if (!isAgent) {
        ...
        respondError('kickbase challenges lobby archive', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  social
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/socialhub');
        if (!isAgent) {
          if (result.shme) console.log(`You: ${result.shme.unm ?? 'unknown'} (${result.shme.skp ?? 0} skill points)`);
        ...
        respondError('kickbase challenges social overview', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  social
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/socialhub/search');
        if (!isAgent && result.it) {
          formatTable(result.it, [
        ...
        respondError('kickbase challenges social search', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  social
    ...
    .action(async () => {
      try {
        const result = await client.get<any>('/v4/challenges/socialhub/invitation');
        if (!isAgent) {
        ...
        respondError('kickbase challenges social invitations', err.message, 'FETCH_FAILED',
          'Ensure you are logged in. Run: kickbase user login');
      }
    });
  ...
  social
    ...
    .action(async (userId: string) => {
      try {
        const result = await client.post<any>('/v4/challenges/socialhub', { ui: userId });
        if (!isAgent) console.log(`Now following ${userId}.`);
        respond('kickbase challenges social follow', result, [
          { command: 'kickbase challenges social overview', description: 'View social hub' },
        ]);
      } catch (err: any) {
        respondError('kickbase challenges social follow', err.message, 'ACTION_FAILED',
          `Check that user ID "${userId}" is valid.`);
      }
    });
  ...
  social
    ...
    .action(async (userId: string) => {
      try {
        const result = await client.delete<any>(`/v4/challenges/socialhub/${userId}`);
        if (!isAgent) console.log(`Unfollowed ${userId}.`);
        respond('kickbase challenges social unfollow', result, [
          { command: 'kickbase challenges social overview', description: 'View social hub' },
        ]);
      } catch (err: any) {
        respondError('kickbase challenges social unfollow', err.message, 'ACTION_FAILED',
          `Check that user ID "${userId}" is valid.`);
      }
    });
}
