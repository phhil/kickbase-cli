...
export const TEAMS: Record<string, { slug: string; id: number; short: string }> = {
...
async function fetchPage(path: string): Promise<HTMLElement> {
  ...
  if (!res.ok) throw new Error(`LigaInsider returned ${res.status} for ${path}`);
...
export async function fetchNews(page = 1): Promise<NewsItem[]> {
  ...
  const articleGroups = new Map<string, { texts: string[]; href: string }>();
  ...
  for (const img of imgs) {
    ...
    let status = '';
    if (src.includes('verletzung')) status = 'Verletzung';
    else if (src.includes('aufbautraining')) status = 'Aufbautraining';
    else if (src.includes('angeschlagen')) status = 'Angeschlagen';
    else if (src.includes('gesperrt') || src.includes('rote-karte')) status = 'Gesperrt';
    else continue;
  ...
  for (const [href, group] of articleGroups) {
    ...
    if (title === player && texts.length === 1) continue; // Skip entries with only one text
...
function resolveTeam(input: string): { slug: string; id: number } {
...
export async function fetchTeamPage(teamInput: string): Promise<TeamLineup> {
  ...
  if (matchEl) nextMatch = matchEl.textContent?.trim() ?? '';
  ...
  for (const link of playerLinks) {
    ...
    for (const icon of icons) {
      ...
      if (src.includes('verletzung')) status = 'Verletzt';
      else if (src.includes('aufbautraining')) status = 'Aufbautraining';
      else if (src.includes('angeschlagen')) status = 'Angeschlagen';
      else if (src.includes('gesperrt') || src.includes('rote-karte')) status = 'Gesperrt';
    }
...
export async function fetchAllInjuries(): Promise<TeamInjuries[]> {
...
export async function fetchTable(): Promise<TableEntry[]> {
...
export async function fetchPlayer(slug: string): Promise<PlayerProfile> {
