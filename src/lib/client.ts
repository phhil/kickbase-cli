...
export class KickbaseClient {
  ...
  private saveToken(token: string, expires: string): void {
  ...
  async login(email?: string, password?: string): Promise<any> {
    ...
    const result = await this.doRequest<any>('POST', '/v4/user/login', {
      em,
      pass,
      loy: false,
      rep: {},
  ...
  private async refreshToken(): Promise<void> {
    const result = await this.doRequest<any>('POST', '/v4/user/refreshtokens');
    if (result.tkn) {
      this.token = result.tkn;
      this.saveToken(result.tkn, result.tknex);
    } else {
  ...
  async cachedGet<T>(path: string, ttl: number): Promise<T> {
